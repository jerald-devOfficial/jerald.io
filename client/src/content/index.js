  ...
  blog: {
    ...
    tailwindcssandreact: {
      ...
      content: [
        ...
        "The above code is from tailwind’s guide but with recent tests, this code won’t work. This is probably caused by PostCSS compatibility with Create React App. If you try that you may encounter with this error with PostCSS on your terminal: ",
        ...
        "After craco is installed, locate your package.json to update the scripts and replace react scripts with craco scripts. Red background scripts should be replaced with the green backgroud scripts which is craco, as shown below.",
        ...
        "Remove the previous versions of autoprefixer, postcss, and tailwindcss, like the ones on red backgroud. Then replace it with the versions as shown on the green backgrounds.",
        "Next, delete the node_modules folder on your my-app folder. This needs to be done so that we can reinstall a clean node_modules that comes from the 'package.json'",
        "Then on your terminal, reinstall node_modules by:",
        "This setup is one of the fixes on installing both Create React App and TailwindCSS 2.0. The following steps below are additional changes to tailwindcss setup. Just here, the errors are already fixed. You can now try running the react app anytime with no error just by:",
        ...
        "Finally, make sure that index.css is being imported to './src/index.js' file. It is normally included inside index.js of your React App, if so there's no need to modify the 'index.js' just to import 'index.css'",
      ],
    },
  },
};
